...
namespace Contact.Api.Queue
{
    public class ProducerManager : IProducerManager
    {
        ...
        private readonly IObjectFormatConverter _objectConvertFormat;
        private dynamic _channel;
        private dynamic _connection;

        public ProducerManager(IRabbitMQService rabbitMQServices, IObjectFormatConverter objectConvertFormat)
        {
        ...
        public bool Enqueue<T>(IEnumerable<T> queueDataModels, string queueName) where T : class, new()
        {
            bool retVal = true;
            try
            {
